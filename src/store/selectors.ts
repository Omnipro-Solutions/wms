...
export function selectDashboardKpis(state: WmsState): DashboardKpis {
  ...
  const pendingReceipts = state.asnRecords.filter((a) => a.status === "pending" || a.status === "partial").length;
  ...
  const activeRoutes = state.sapRoutes.filter((r) => r.status === "in_transit" || r.status === "in_progress").length;
