...
const STATUS_CONFIG: Record<IntegrationConnection["status"], { label: string; icon: React.ReactNode; className: string }> = {
  active: {
    label: "Activa",
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <Card className={errorCount > 0 ? "border-red-200 bg-red-50" : ""}>
          <CardContent className="pt-6">
            ...
            <p className={`text-2xl font-bold tabular-nums ${errorCount > 0 ? "text-red-700" : ""}`}>
        ...
        <Card>
          <CardContent className="pt-6">
            ...
            <p className="text-2xl font-bold tabular-nums text-green-700">{formatNumber(activeCount)}</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-6">
        ...
        <Card className="border-red-200 bg-red-50/50">
          <CardContent className="pt-4 pb-4">
            ...
            <div className="mt-2 space-y-1">
              {state.integrations.filter((i) => i.status === "error").map((i) => (
                <p key={i.id} className="text-sm text-red-600">
                  <span className="font-medium">{i.name}:</span> {i.lastError ?? "Error desconocido"}
                </p>
              ))}
            </div>
          </CardContent>
            ...
            <Card key={integration.id} className={`border ${cfg.className}`}>
              ...
              <CardContent className="space-y-2">
                ...
                <div className="flex items-center justify-between text-sm">
                  ...
                  <span className="font-medium tabular-nums">{formatNumber(integration.processedMessages)}</span>
                </div>
                ...
                {integration.lastError && (
                  <p className="rounded bg-red-100 px-2 py-1 text-xs text-red-700">{integration.lastError}</p>
                )}
            ...
            <TableBody>
              {state.integrations.map((integration) => (
                <TableRow key={integration.id}>
                  ...
                  <TableCell className="text-right tabular-nums">{formatNumber(integration.processedMessages)}</TableCell>
