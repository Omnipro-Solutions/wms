...
export const buildSapRouteColumns = (): ColumnDef<SapRouteRow>[] => [
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <span className="text-sm">{row.getValue("originName")}</span>
    ),
  },
  {
    id: "destinations",
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <div>
        ...
        <p className="font-mono text-xs text-muted-foreground">{row.original.truckPlate}</p>
      </div>
    ),
    enableSorting: false,
  },
  {
    ...
    header: ({ column }) => (
      <DataTableColumnHeader column={column} title="Fecha ruta" />
    ),
    cell: ({ row }) => (
      <span className="text-sm">{row.getValue("routeDate")}</span>
    ),
  },
  {
    accessorKey: "loadPct",
      ...
      return (
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs tabular-nums text-muted-foreground">
            {formatNumber(row.original.currentLoadKg)}/
            {formatNumber(row.original.capacityKg)} kg
          </span>
        </div>
      )
    },
