...
import { CheckCircle2, PlayCircle, TriangleAlert, Waves } from 'lucide-react'
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <Card>
          <CardContent className="pt-6">
            <p className="text-muted-foreground text-sm">Oleadas activas</p>
        ...
        <Card>
          <CardContent className="pt-6">
            <p className="text-muted-foreground text-sm">En borrador</p>
        ...
        <Card>
          <CardContent className="pt-6">
            <p className="text-muted-foreground text-sm">Unidades en oleadas activas</p>
                ...
                return (
                  <TableRow key={wave.id}>
                    ...
                    <TableCell>
                      <Badge
                        variant="outline"
                        className={`text-xs ${PRIORITY_COLORS[wave.priority]}`}
                      >
                        {PRIORITY_LABELS[wave.priority]}
                      </Badge>
                    </TableCell>
                    <TableCell className="text-right tabular-nums">
                      {formatNumber(wave.orderCount)}
                    </TableCell>
                    <TableCell className="text-right tabular-nums">
                      {formatNumber(wave.unitCount)}
                    </TableCell>
                    <TableCell className="text-muted-foreground text-sm">
                      {wave.assignedTeam ?? '—'}
                    </TableCell>
          ...
          {releaseDialog.data && (
            <div className="space-y-4 py-2">
              <div className="text-muted-foreground space-y-1 text-sm">
                <p>
                  Oleada:{' '}
                  <span className="text-foreground font-medium">{releaseDialog.data.code}</span>
                </p>
                <p>
                  Nombre:{' '}
              ...
              <p className="text-muted-foreground text-sm">
                Liberar cambiará el estado de la oleada a <strong>En progreso</strong> y habilitará
                las tareas de picking asociadas.
              </p>
              {releaseDialog.error && (
