...
export const buildReturnColumns = (
...
): ColumnDef<ReturnRow>[] => [
  {
    accessorKey: "rmaCode",
    header: ({ column }) => (
      <DataTableColumnHeader column={column} title="RMA" />
    ),
    cell: ({ row }) => (
      <span className="font-medium">{row.getValue("rmaCode")}</span>
    ),
  },
  {
  ...
  },
  {
    ...
    header: ({ column }) => (
      <DataTableColumnHeader column={column} title="Disposición" />
    ),
    cell: ({ row }) => {
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return (
        <Button
          size="sm"
          onClick={(e) => { e.stopPropagation(); onAdvance(row.original) }}
        >
          Avanzar
        </Button>
      )
