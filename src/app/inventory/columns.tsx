...
export const buildInventoryColumns = (
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <span className="font-mono text-xs">{row.getValue("locationCode")}</span>
    ),
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return (
        <Badge
          variant={cls === "A" ? "default" : cls === "B" ? "secondary" : "outline"}
        >
          {cls}
        </Badge>
      )
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <div className="text-right tabular-nums">
        {formatNumber(row.getValue("onHandQuantity"))}
      </div>
    ),
  },
  {
    ...
    cell: ({ row }) => (
      <div className="text-right tabular-nums text-blue-600">
        ...
        <div className="flex gap-1">
          {item.status !== "on_hold" && (
            <Button
              size="sm"
              variant="outline"
              onClick={(e) => { e.stopPropagation(); onAction("hold", item) }}
            >
              Hold
            </Button>
          )}
          {item.holdQuantity > 0 && (
            <Button
              size="sm"
              variant="outline"
              onClick={(e) => { e.stopPropagation(); onAction("release", item) }}
            >
              Liberar
            </Button>
          )}
          <Button
            size="sm"
            variant="ghost"
            onClick={(e) => { e.stopPropagation(); onAction("adjust", item) }}
          >
            Ajustar
          </Button>
        </div>
