...
import { CheckCircle2, Package, ScanLine, Tag, TriangleAlert } from 'lucide-react'
...
export default function PackingPage() {
  ...
  const handleCompletePacking = () => {
    ...
    if (isNaN(n) || n < 0) {
      packDialog.setError('Ingresa una cantidad válida.')
      return
    }
    try {
  ...
  const verifiedCount = state.packingOrders.filter(
    (p) => p.verificationStatus === 'verified'
  ).length
  const mismatchCount = state.packingOrders.filter(
    (p) => p.verificationStatus === 'mismatch'
  ).length
  ...
  return (
    <>
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <Card>
          <CardContent className="pt-6">
            <p className="text-muted-foreground text-sm">Pendientes de verificar</p>
        ...
        <Card>
          <CardContent className="pt-6">
            <p className="text-muted-foreground text-sm">Con discrepancia</p>
          ...
          <Table>
            ...
            <TableBody>
              {state.packingOrders.map((pk) => {
                const progress =
                  pk.expectedItems > 0 ? Math.round((pk.scannedItems / pk.expectedItems) * 100) : 0
                ...
                return (
                  <TableRow key={pk.id}>
                    ...
                    <TableCell className="text-right tabular-nums">
                      {formatNumber(pk.expectedItems)}
                    </TableCell>
                    <TableCell className="text-right tabular-nums">
                      {formatNumber(pk.scannedItems)}
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell>
                      <Badge
                        variant="outline"
                        className={`text-xs ${VERIFICATION_COLORS[pk.verificationStatus]}`}
                      >
      ...
      <Dialog
        open={!!packDialog.data}
        onOpenChange={(o) => {
          if (!o) packDialog.close()
        }}
      >
        <DialogContent>
          <DialogHeader>
          ...
          {packDialog.data && (
            <div className="space-y-4 py-2">
              <div className="text-muted-foreground space-y-1 text-sm">
                <p>
                  ...
                  <span className="text-foreground font-medium">
                    {packDialog.data.customerName}
                  </span>
                </p>
                <p>
                  Caja sugerida:{' '}
                  <span className="text-foreground font-medium">
                    {packDialog.data.suggestedBox}
                  </span>
                </p>
                <p>
                  Ítems esperados:{' '}
                  <span className="text-foreground font-medium">
                    {packDialog.data.expectedItems}
                  </span>
                </p>
              </div>
              ...
              {packDialog.data &&
                parseInt(scannedQty, 10) !== packDialog.data.expectedItems &&
                scannedQty !== '' && (
                  <div className="flex items-center gap-2 rounded-md border border-amber-200 bg-amber-50 p-3 text-sm text-amber-700">
                    <TriangleAlert className="size-4 shrink-0" />
                    <p>
                      Discrepancia detectada: esperado {packDialog.data.expectedItems}, escaneado{' '}
                      {scannedQty}. Se registrará como <strong>mismatch</strong>.
                    </p>
                  </div>
                )}
              {packDialog.error && (
          ...
          <DialogFooter>
            <Button variant="outline" onClick={packDialog.close}>
              Cancelar
            </Button>
