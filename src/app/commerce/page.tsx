...
export default function CommercePage() {
  ...
  const sorted = [...filtered].sort((a, b) => {
    ...
    if (sortField === "delivery") return a.promisedDeliveryDate.localeCompare(b.promisedDeliveryDate) * d;
  ...
  const openReserveDialog = (orderId: string) => {
    ...
    reserveDialog.open({
      ...
      lines: order.items.map((l) => ({ productId: l.productId, requestedQuantity: l.requestedQuantity })),
    });
  };
  ...
  return (
    <>
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <Card>
          <CardContent className="pt-6">
            <p className="text-sm text-muted-foreground">Pendientes de reserva</p>
        ...
        <Card>
          <CardContent className="pt-6">
            ...
            <p className="text-2xl font-bold tabular-nums text-blue-600">{formatNumber(inProgressCount)}</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-6">
      ...
      <Dialog open={!!reserveDialog.data} onOpenChange={(o) => { if (!o) reserveDialog.close(); }}>
        <DialogContent>
          <DialogHeader>
            ...
            <div className="space-y-4 py-2">
              <div className="space-y-1">
                <p className="text-sm text-muted-foreground">
                  Pedido: <span className="font-medium text-foreground">{reserveDialog.data.orderNumber}</span>
                </p>
                <p className="text-sm text-muted-foreground">
                  Cliente: <span className="font-medium text-foreground">{reserveDialog.data.customerName}</span>
                </p>
              </div>
                  ...
                  <TableBody>
                    {reserveDialog.data.lines.map((l) => (
                      <TableRow key={l.productId}>
                        ...
                        <TableCell className="text-right tabular-nums">{l.requestedQuantity}</TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                </Table>
              ...
              <p className="text-sm text-muted-foreground">
                Esta acción reservará el stock disponible y cambiará el pedido a estado <strong>Reservado</strong>.
              </p>
              {reserveDialog.error && (
          ...
          <DialogFooter>
            <Button variant="outline" onClick={reserveDialog.close}>Cancelar</Button>
