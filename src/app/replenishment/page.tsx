...
import { AlertTriangle, CheckCircle2, PlayCircle, TriangleAlert, Warehouse } from 'lucide-react'
...
export default function ReplenishmentPage() {
  ...
  const highCount = state.replenishmentTasks.filter(
  ...
  return (
    <>
          ...
          <Table>
            ...
            <TableBody>
              {sorted.map((task) => {
                ...
                const stockPercent =
                  task.maxStock > 0 ? Math.round((task.currentStock / task.maxStock) * 100) : 0

                return (
                  <TableRow
                    key={task.id}
                    className={
                      livePriority === 'high' && task.status !== 'completed' ? 'bg-red-50/40' : ''
                    }
                  >
                    ...
                    <TableCell>
                      <Badge
                        variant="outline"
                        className={`text-xs ${PRIORITY_COLORS[livePriority]}`}
                      >
                        {PRIORITY_LABELS[livePriority]}
                      </Badge>
                    </TableCell>
                    <TableCell className="font-mono text-xs">
                      {locationCode(task.originLocationId)}
                    </TableCell>
                    <TableCell className="font-mono text-xs">
                      {locationCode(task.destinationLocationId)}
                    </TableCell>
                    <TableCell
                      className={`text-right tabular-nums ${needsRep && task.status !== 'completed' ? 'font-semibold text-red-600' : ''}`}
                    >
                      {formatNumber(task.currentStock)}
                    </TableCell>
                    <TableCell className="text-muted-foreground text-right tabular-nums">
                      {formatNumber(task.minStock)}
                    </TableCell>
                    <TableCell className="text-muted-foreground text-right tabular-nums">
                      {formatNumber(task.maxStock)}
                    </TableCell>
                    <TableCell className="text-right font-semibold tabular-nums">
                      {formatNumber(task.suggestedQuantity)}
                    </TableCell>
                    <TableCell>
                      ...
                      <div className="flex gap-1">
                        {task.status === 'pending' && (
                          <Button
                            size="sm"
                            variant="outline"
                            onClick={() => openTaskDialog(task, 'start')}
                          >
      ...
      <Dialog
        ...
        onOpenChange={(o) => {
          if (!o) taskDialog.close()
        }}
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {taskDialog.data?.action === 'start'
                ? 'Asignar tarea de reposición'
          ...
          {taskDialog.data && (
            <div className="space-y-4 py-2">
              <div className="text-muted-foreground space-y-1 text-sm">
                <p>
                  Producto:{' '}
                  <span className="text-foreground font-medium">{taskDialog.data.productName}</span>
                </p>
                <p>
                  Desde:{' '}
                  <span className="text-foreground font-mono font-medium">
                    {taskDialog.data.fromCode}
                  </span>
                </p>
                <p>
                  Hacia:{' '}
                  <span className="text-foreground font-mono font-medium">
                    {taskDialog.data.toCode}
                  </span>
                </p>
                <p>
                  Cantidad sugerida:{' '}
                  <span className="text-foreground font-medium">
                    {taskDialog.data.suggestedQty} uds.
                  </span>
                </p>
              </div>
              {taskDialog.data.action === 'complete' && (
                <p className="text-muted-foreground text-sm">
                  Se moverán <strong>{taskDialog.data.suggestedQty} uds.</strong> desde la reserva
                  al pick face y se registrará el movimiento en el log de auditoría.
                </p>
              )}
              {taskDialog.error && (
          ...
          <DialogFooter>
            <Button variant="outline" onClick={taskDialog.close}>
              Cancelar
            </Button>
